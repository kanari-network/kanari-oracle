...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Price { symbol, asset_type, config } => {
            get_single_price(symbol, asset_type, config).await
        }
        Commands::List { asset_type, config } => {
            list_symbols(asset_type, config).await
        }
...
async fn get_single_price(symbol: String, asset_type: String, config_path: String) -> Result<()> {
    ...
    let price = match asset_type.as_str() {
        "crypto" => {
            let available: HashSet<String> = oracle.get_crypto_symbols().into_iter().map(|s| s.to_lowercase()).collect();
            if !available.contains(&symbol.to_lowercase()) {
                error!("Symbol '{}' not configured for crypto", symbol);
                return Ok(());
            }
            oracle.get_crypto_price(&symbol)
                .await
                .context("Failed to fetch crypto price")?
        }
        "stock" => {
            let available: HashSet<String> = oracle.get_stock_symbols().into_iter().map(|s| s.to_uppercase()).collect();
            if !available.contains(&symbol.to_uppercase()) {
                error!("Symbol '{}' not configured for stock", symbol);
                return Ok(());
            }
            oracle.get_stock_price(&symbol)
                .await
