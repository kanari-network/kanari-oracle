...
impl CryptoFetcher {
        ...
        if let Some(obj) = response.as_object() {
            for (coin_id, data) in obj {
                if let Some(price_obj) = data.as_object() {
                    ...
                    // Use direct change value from API instead of calculating
                    let change_24h = price_obj.get(&format!("{}_24h_change", vs_currency))
                        .and_then(|c| c.as_f64());
                    
                    let change_24h_percent = price_obj.get(&format!("{}_24h_change", vs_currency))
                        .and_then(|c| c.as_f64());
                    
                    // If change_24h is not available, calculate from percentage as fallback
                    let final_change_24h = if let Some(c) = change_24h {
                        Some(c)
                    } else if let Some(pct) = change_24h_percent {
                        Some((price * pct) / 100.0)
                    } else {
                        None
                    };
                    ...
                    price_data.change_24h = final_change_24h;
    ...
    async fn fetch_binance_24hr_ticker(&self, original_symbol: &str) -> Result<PriceData> {
        ...
        self.fetcher.retry_with_backoff(|| async {
            ...
            if !response.status().is_success() {
                return Err(OracleError::ApiError(
                    format!("Binance 24hr API error: {}", response.status())
                ));
            }
            
            let ticker_data: serde_json::Value = response.json().await?;
            
            debug!("Binance 24hr ticker response: {}", serde_json::to_string_pretty(&ticker_data).unwrap_or_default());
            
            let price: f64 = ticker_data["lastPrice"].as_str()
                .and_then(|s| s.parse().ok())
                .ok_or_else(|| {
                    OracleError::ApiError(format!(
                        "Invalid price format from Binance 24hr. Response: {}", 
            ...
            info!("Parsed Binance 24hr data: price={}, change={}, change%={}", price, price_change, price_change_percent);
    ...
    async fn fetch_binance_price_only(&self, symbol: &str) -> Result<PriceData> {
        ...
        info!("Fetching Binance price only for: {} (URL: {})", binance_symbol, url);
        ...
        self.fetcher.retry_with_backoff(|| async {
            ...
            if !response.status().is_success() {
                return Err(OracleError::ApiError(
                    format!("Binance price API error: {}", response.status())
                ));
            }
            
            let price_data: serde_json::Value = response.json().await?;
            
            let price: f64 = price_data["price"].as_str()
                .and_then(|s| s.parse().ok())
                .ok_or_else(|| OracleError::ApiError(format!(
                    "Invalid price from Binance for {}: {}", symbol, price_data
                )))?;
    ...
    pub async fn fetch_all_crypto_prices(&self) -> Result<Vec<PriceData>> {
        ...
        // Try Binance for missing symbols individually with fallback handling
        let existing_symbols: HashSet<String> = all_prices.iter()
            .map(|p| p.symbol.clone())
            .collect();
        
        let missing_symbols: Vec<String> = symbols.iter()
            .filter(|s| !existing_symbols.contains(&s.to_lowercase()))
            .cloned()
            .collect();
        
        for symbol in &missing_symbols {
            // Try different Binance endpoints in order of preference
            let price_result = match self.fetch_binance_24hr_ticker(symbol).await {
            ...
            match price_result {
                Ok(price_data) => {
                    info!("Successfully fetched {} price from Binance: ${:.2}", symbol, price_data.price);
                    all_prices.push(price_data);
                }
                Err(e) => {
                    error!("All Binance APIs failed for {}: {}", symbol, e);
                }
            }
        }
        
        if all_prices.is_empty() {
            return Err(OracleError::ApiError("All crypto price sources failed".to_string()));
        }
        
        info!("Successfully fetched {} total crypto prices", all_prices.len());
        Ok(all_prices)
    }
    ...
    pub async fn fetch_binance_prices(&self, symbols: &[String]) -> Result<Vec<PriceData>> {
        ...
        for symbol in symbols {
            ...
            match price_result {
                Ok(price_data) => {
                    info!("Successfully fetched {} from Binance: ${:.2}", symbol, price_data.price);
                    prices.push(price_data);
                }
                Err(e) => {
                    error!("All Binance methods failed for {}: {}", symbol, e);
                }
            }
        }
        
        if prices.is_empty() && !symbols.is_empty() {
            return Err(OracleError::ApiError("Failed to fetch any prices from Binance".to_string()));
        }
